...
class MMJWTourney(Peer):
    ...
    def uploads(self, requests, peers, history):
        ...
        if len(requests) == 0:
            ...
            ####
            # Modification of propshare, where we scale the optimistically
            # unchoked portion to have the same bandwidth as client c',
            # where c' is the client that has the minimum bandwidth among
            # those we deterministically choose
            ...
            adjusted_bws_sum = 0
            optim_candidates = list(set(optim_candidates))
            if len(optim_candidates) > 0: # add same weight
                optim_id = random.choice(optim_candidates)
                # note here we can use a dummy value in the else because
                # it is only reached if we have deterministically chosen 0
                # i.e. we have not had any downloads in the past
                chosen_req_bw[optim_id] = min(chosen_req_bw.values()) if\
                        len(chosen_req_bw) > 0 else 1
                tot_bws += chosen_req_bw[optim_id]

            for i in chosen_req_bw.keys():
                chosen_req_bw[i] *= self.up_bw
            ...
            for i in random.sample(list(chosen_req_bw.keys()), k = max(0, rem)):
                chosen_req_bw[i] += 1
